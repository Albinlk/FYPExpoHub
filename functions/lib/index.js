...
exports.processMasterFileImport = void 0;
 ...
 */
function normaliseMalayDate(dateStr) {
...
// -----------------------------------------------------------------
exports.processMasterFileImport = functions.storage.object().onFinalize(async (object) => {
    const filePath = object.name;
    if (!filePath || !filePath.startsWith('private/imports/'))
        return;
    ...
    try {
        ...
        const summary = {};
        const warningCounts = { overlap: 0, privacy: 0, format: 0 };
        ...
        if (sheetNames.includes('PEMENANG ANUGERAH')) {
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                if (row['STUDENT_ID'] || row['MATRIK'] || row['NO_TEL'] || row['EMAIL']) {
                    if (row['STUDENT_ID'] || row['MATRIK'])
                        emailSkipCount++;
                    if (row['NO_TEL'] || row['EMAIL'])
                        phoneSkipCount++;
                }
